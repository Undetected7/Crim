-- GRIMOIRE.CC - UI (полноценное меню)
return function()
    local UI = {}
    ...
    function BuildVisualsTab()
        ...
        -- Создаём градиент (просто для красоты)
        for i = 0, 1, 0.05 do
    ...
    function BuildFeaturesTab()
        ...
        UIS.InputChanged:Connect(function(input)
            if draggingBright and input.UserInputType == Enum.UserInputType.MousePosition then
                ...
                -- Применяем сразу, если включен Fullbright
    ...
    -- Вкладка Settings (бонус: список друзей, перезагрузка)
    function BuildSettingsTab()
        ...
        reloadBtn.MouseButton1Click:Connect(function()
            -- Простая перезагрузка путём повторного вызова loadstring (можно реализовать полную)
            print("[GRIMOIRE] Перезагрузка модулей...")
            local loader = _G.GRIMOIRE["Loader.lua"]
            if loader and loader.Reload then
                loader.Reload()
            else
                warn("Функция Reload не реализована, просто перезапусти скрипт.")
            end
        end)
    end
    ...
    -- Глобальная функция переключения видимости
    ...
    -- Возвращаем API
    return {
        CreateUI = function() end, -- уже создано
