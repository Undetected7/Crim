...
return function(Config)
    ...
    local function drawBox(plr, color, overlay)
    ...
    local function StorageESP(cfg, overlay)
        if not cfg or not cfg.Toggles.Storage then return end
    ...
    function Visuals.Start(cfg)
        cfg = cfg or Config
        if not cfg then error("[GRIMOIRE] Visuals: Config не передан") end
        ...
        task.spawn(function()
            while true do
                if cfg and cfg.Toggles.ESP then
                    local color = cfg.Colors.ESP or Color3.new(1,0,0)
                    for _, plr in ipairs(getPlayers()) do
                        drawBox(plr, color, overlay)
                    end
                    StorageESP(cfg, overlay)
                end
                task.wait()
            end
        end)
